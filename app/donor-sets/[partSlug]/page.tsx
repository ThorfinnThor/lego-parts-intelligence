...
import { getDonorData, getDonorPageParams } from '@/lib/data/static-catalogue';
...
export default async function DonorPage({ params }: { params: Promise<{ partSlug: string }> }) {
  ...
  return (
    <div className="shell page-shell">
      <Breadcrumbs items={[{ label: 'Home', href: '/' }, { label: 'Parts', href: '/parts/' }, { label: data.partName, href: `/parts/${data.partSlug}/` }, { label: 'Donor sets' }]} />
      ...
      <section className="panel">
        <div className="table-wrap"><table><thead><tr><th>Rank</th><th>Set</th><th>Year</th><th className="number">Target qty.</th><th className="number">Total units</th><th className="number">Density</th><th className="number">Score</th></tr></thead><tbody>
          {data.candidates.map((candidate, index) => <tr key={candidate.setId}><td>{index + 1}</td><td><Link href={`/sets/${candidate.setSlug}/`}>{candidate.setName} <small>{candidate.setId}</small></Link></td><td>{candidate.year ?? '—'}</td><td className="number">{candidate.targetQuantity}</td><td className="number">{candidate.setTotalParts}</td><td className="number">{Math.round(candidate.partDensity * 100)}%</td><td className="number"><strong>{Math.round(candidate.inventoryDonorScore * 100)}</strong></td></tr>)}
        </tbody></table></div>
      </section>
